...
import static org.flywaydb.community.database.dm.DMSchema.ObjectType.*;
    ...
    public enum ObjectType {
         ...
         */
         public static Set<String> getObjectTypeNames(JdbcTemplate jdbcTemplate, DMDatabase database, DMSchema schema) throws SQLException {
            boolean xmlDbAvailable = database.isXmlDbAvailable();
            String query = "SELECT DISTINCT OBJECT_TYPE FROM " + database.dbaOrAll("OBJECTS") + " WHERE OWNER = ? UNION SELECT '" + DATABASE_LINK.getName() + "' FROM DUAL WHERE EXISTS(SELECT * FROM " + database.dbaOrAll("DB_LINKS") + " WHERE OWNER = ?) ";
            int n = 1 + (xmlDbAvailable ? 1 : 0) + 1;
            String[] params = new String[n];
            Arrays.fill(params, schema.getName());
        ...
        public static boolean supportedTypesExist(JdbcTemplate jdbcTemplate, DMDatabase database, DMSchema schema) throws SQLException {
            ...
            Arrays.asList(
                    ...
                    ASSEMBLY.getName(),
                    JAVA_DATA.getName())
                    .forEach(existingTypeNames::remove);
